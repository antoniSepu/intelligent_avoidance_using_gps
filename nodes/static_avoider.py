...
class Avoider(Camera): #Class which inheritance from Camera
    ...
    def run(self): #Main loop
        ...
        while not rospy.is_shutdown():
            ...
            elif selself._state.mode == 'HOLD' and threat == False:
    ...
    def _check_front(self, scan): #Return True if there is risk of collision
        ...
        for i in range(len(scan)):
            if scan[i] <= 2.5 and isnan(scan[i]) == False and i >= 272 and i <= 356:
                threat = True
            elif scan[i] <= 1.5 and isnan(scan[i]) == False and i >= 228 and i <= 406:
                threat = True
            elif scan[i] <= 1 and isnan(scan[i]) == False and i >= 166 and i <= 456:
                threat = True
    ...
    def _where_2_go(self, scan):#This method decide where to avoid
        ...
        if proximity_alert:
            ...
            #Nos vamos para atras
            return 0
        elif dif >= 50:
            #Nos vamos para la derecha
            return 1
        elif dif <= -50:
