...
from math import pow, sqrt, sin, cos, atan2, pi, radians, degrees
...
class Marcadores(Camera):
    def __init__(self):
        ...
        self._sub_wp = rospy.Subscriber('/mavros/mission/waypoints', WaypointList, self._update_wp)

        self._marker_array = MarkerArray()
        self._mag_in = Float64()
        self._wp_in = WaypointList()
        self._marker_array.markers = []
        self._wp_in.waypoints = []
    ...
    def _generar_marker_objetivo(self): #cubo verde
        self._marker = Marker()
        #print(self._wp_in)
        try:
            lat = self._wp_in.waypoints[2].x_lat
            long = self._wp_in.waypoints[2].y_long
            #print("Wp recibido 1")
        except IndexError:
            print("IndexError 1")
            lat = 40.474146
        ...
        (northing, easting, utmzone) = LLtoUTM(lat, long)
        ...
        self._marker.pose.orientation.z = z
        self._marker.pose.orientation.w = w
        self._marker.pose.position.x = easting
        self._marker.pose.position.y = northing
        self._marker.pose.position.z = 0

        self._marker_array.markers.append(self._marker)

    def _generar_flecha_objetivo(self): #Flecha verde
        try:
            lat = self._wp_in.waypoints[2].x_lat
            long = self._wp_in.waypoints[2].y_long
            #print("Wp recibido 2")
        ...
        #print("{0}: is the lat. {1}: is the long.".format(lat,long))
        (northing, easting, utmzone) = LLtoUTM(lat, long)

        self._marker = Marker()
        self._angle_objective = radians(self._mag_in.data) + atan2(northing, easting)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self._generar_marker_objetivo()
            self._generar_flecha_objetivo()
    ...
    def _update_wp(self, data):
        self._wp_in = data

def main():
    rospy.init_node('Marker_Drawer')
